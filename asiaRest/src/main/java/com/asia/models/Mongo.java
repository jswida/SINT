...
import static com.asia.DataBase.*;
...
public class Mongo {
    ...
    private Mongo() {
        ...
        datastore = morphia.createDatastore(new MongoClient("localhost", 8004), "sint_database");
    ...
    public void loadMango() {
        ...
        if (datastore.getCount(Course.class) <= 0 && datastore.getCount(Student.class) <= 0) {
            ...
            try {
                ...
                Course course2 = generateCourse("PIRO");
                course2.setId(nextCourseId());

                Course course3 = generateCourse("MISIO");
                course3.setId(nextCourseId());

                Student student1 = generateStudent("Jan", "Nowak");
                student1.setIndex(nextStudentId());
                Student student2 = generateStudent("Stanisław", "Szatniak");
                student2.setIndex(nextStudentId());
                Student student3 = generateStudent("Michał", "Grazikowski");
                ...
                System.out.println("Error - init data created");
                e.printStackTrace();
            }
        }
    }
